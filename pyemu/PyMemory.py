...
class PyMemory(object):
    ...
    def clear_memory(self, address, size):
        self.set_memory(address, "\x00"*size, size, False)

    #
    # set_memory: Set an address to a specific value.  This can be a
    #             integer or string.
    #
    def set_memory(self, address, value, size, updateStats=True):
        page = address & 0xfffff000
        offset = address & 0x00000fff
...
class PEMemory(PyMemory):
    ...
    def allocate(self, size):
        ...
        #pagenum = num
        #
        #address = self.get_available_page()
        #if not self.allocate_page(address):
